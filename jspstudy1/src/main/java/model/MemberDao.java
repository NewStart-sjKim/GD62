...
public class MemberDao {
	...
	public String idSearch(String email, String tel) {
		...
		try {
			...
			// rs.next() : select된 결과 정보 조회.
			if(rs.next()) {
				return rs.getString("id"); //입력된 id값에 해당하는 정보를 db에서 조회된 데이터 저장
			}
		}catch(SQLException e) {
			e.printStackTrace();
		}finally {
	...
	public String pwSearch(String id, String email, String tel) {
		...
		try {
			...
			rs = pstmt.executeQuery(); //데이터 한개만 가져온다
			// rs.next() : select된 결과 정보 조회.
			if(rs.next()) {
				return rs.getString("pass"); //입력된 pass값에 해당하는 정보를 db에서 조회된 데이터 저장
			}
		}catch(SQLException e) {
			e.printStackTrace();
		}finally {
	...
	public boolean updatePass(String id, String pass) {
		...
		try {
			...
			return pstmt.executeUpdate()>0; //데이터 한개만 가져온다		
		}catch(SQLException e) {
			e.printStackTrace();
		}finally {
			DBConnection.close(conn, pstmt, null);
