...
public class BoardController extends MskimRequestMapping {
   ...
   5. 첨부파일이 있는 경우 @ 표시하기
	...
	public String info(HttpServletRequest request,HttpServletResponse response) {
		...
		String boardid = (String)request.getSession().getAttribute("boardid");
		if(boardid == null) boardid = "1";
		//b : board 테이블에서 num(조회하는 게시물 번호)에 해당하는 데이터 저장
	 ...
	 * 1. 원글의 num을 파라미터 저장 : num 원글의 게시물번호
		2. db에서 num의 게시물 조회하여 원들의 num,grp,grplevel,grpstep,boardid 정보를 저장
		3. 입력 화면 표시
	 */
	...
	/*
 	1. 파라미터 정보들을 Board 객체 저장. => request 객체 사용 불가
	2. 비밀번호 검증
	...
	2. 비밀번호 검증
		틀린경우 : 비밀번호 오류 메세지 출력, deleteForm 페이지 이동
	3. 게시물 삭제.
		해당게시물이 공지사항 게시물인 경우 관리자만 삭제 가능
	4. 게시물 삭제
		boolean BoardDao.delete(num)
		삭제	성공 : list 페이지 이동
		삭제 실패 : 삭제 실패 메세지 출력, info 페이지 이동
	 */
	...
	public String delete(HttpServletRequest request,HttpServletResponse response) {
		...
		if(pass.equals(board.getPass())){ // 비밀번호 맞으면
