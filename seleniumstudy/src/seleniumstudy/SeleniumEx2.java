...
public class SeleniumEx2 {
	public static void main(String[] args) throws Exception {
		...
		String url = "https://search.daum.net/search?w=img&nil_search=btn&DA=NTB&enc=utf8&q=%EC%9E%A5%EB%AF%B8";
		driver.get(url);
		Thread.sleep(1000);
		//findElements : 태그들을 찾는 함수
		//images : img 태그 들
		List<WebElement> images = driver.findElements(By.cssSelector("div.wrap_thumb a > img"));
		...
		for(int i = 0; i < imgurl.size(); i++) {
			...
			InputStream is = new URL(imgurl.get(i)).openConnection().getInputStream();
			//fos : 이미지 저장할 파일
